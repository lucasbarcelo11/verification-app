...
const create = catchError(async(req, res) => {
    ...
    await sendEmail({
        to: email,
        subject: "verificy email for user app",
        html: `
            <h1>Hello ${firstName} ${lastName} </h1>
            <b>Thnaks for sing up in user app</b>
            <br/>
            <a href="${link}">${link}</a>
        `
    })
...
const verifyEmail = catchError(async(req, res) => {
    ...
    if(!emailCode) return res.status(401).json({meesage: "Invalid Code"})
...
const login = catchError(async(req, res) => {
    ...
    if(!user.isVerified) return res.status(401).json({meesage: "User must be verified"})
